...
AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** verbindet sich mit dem Server.', Config.BaseColors['joins'], source, 'joins')
end)

-- Send message when Player disconnects from the server
AddEventHandler('playerDropped', function(reason)
	SinglePlayerLogs('**' ..GetPlayerName(source).. '** hat den Server verlassen. (Reason: ' .. reason .. ')', Config.BaseColors['leaving'], source, 'leaving')
end)
...
AddEventHandler('playerDied',function(id,player,killer,DeathReason, Weapon)
	...
	elseif id == 2 then -- Killed by other player
        ...
        SinglePlayerLogs('**' .. GetPlayerName(source) .. '** `Ist Gestorben`', Config.BaseColors['deaths'], source, 'deaths') -- sending to deaths channel
	end
end)
...
AddEventHandler('playerShotWeapon', function(weapon)
	...
	if Config.weaponLog then
		SinglePlayerLogs('**' .. GetPlayerName(source)  .. '** Feuert mit `' .. weapon .. '`', Config.BaseColors['shooting'], source, 'shooting')
    end
end)
...
AddEventHandler('onResourceStop', function (resourceName)
    HidePlayerDetails('**' .. resourceName .. '** wurde gestoppt.', Config.BaseColors['resources'], 'resources')
end)

-- Send message when a resource is being started
AddEventHandler('onResourceStart', function (resourceName)
    Wait(100)
    HidePlayerDetails('**' .. resourceName .. '** wurde gestartet.', Config.BaseColors['resources'], 'resources')
end)
